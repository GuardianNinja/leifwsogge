from flask import Flask as FlaskType, request, jsonify, Response  # type: ignore[reportUnknownVariableType]
import math
import hashlib
import numpy as np
...
def dummies_override_validate(mayan_input, level_claim):
    # Ensure level_claim is int for dict lookup
    try:
        level_claim_int = int(level_claim)
    except (TypeError, ValueError):
        return False, "invalid_level_type"
    expected = mayan_levels.get(level_claim_int, None)
    if level_claim_int == 26 and mayan_input == mayan_levels[26]:
    ...
    shortened = reversed_hiero_symbols[:min(level_claim_int, len(reversed_hiero_symbols))]
    hash_check = hashlib.sha256(shortened.encode()).hexdigest()[:8]
    
    if mayan_input == expected + hash_check:
        return True, f"Level {level_claim_int} Mayan doodle accepted"
    else:
