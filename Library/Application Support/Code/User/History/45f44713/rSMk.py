...
def plot_distribution(classified_layers):
    ...
    for ax, key, title in zip(axes, elements, titles):  # pyright: ignore[reportUnknownVariableType, reportUnknownArgumentType]
        data = np.asarray(classified_layers[key])  # pyright: ignore[reportUnknownVariableType] # Ensure data is a numpy ndarray
        im = ax.imshow(data, cmap="inferno") # pyright: ignore[reportUnknownVariableType]
