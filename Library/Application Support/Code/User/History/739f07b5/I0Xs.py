...
def test_monte_carlo_runs():
    ...
    assert hasattr(df, '__len__'), f"monte_carlo_trials did not return a sized object, got {type(df)}"
    assert len(df) == 20
    assert hasattr(df, 'columns') and "first_below_0.5" in df.columns # pyright: ignore[reportUnknownMemberType]
