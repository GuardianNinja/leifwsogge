...
class TestTimeEngine(unittest.TestCase):
    ...
    def test_session_tracking(self):
        ...
        self.assertGreater(float(result["session_duration_hours"]), 0.0)
        self.assertGreater(float(result["total_learning_time_hours"]), 0.0)
    ...
    def test_multiple_items_workflow(self):
        ...
        exp_progress: float = analytics["exponential_progress"]  # type: ignore[assignment]
        # If exp_progress is a dict or object, try to extract a float value
        if isinstance(exp_progress, dict):
            # Try common keys
            for key in ("value", "progress", "score"):  # add more if needed
                if key in exp_progress:
                    exp_progress = exp_progress[key]
                    break
        try:
            exp_progress_float: float = float(exp_progress)
        except (TypeError, ValueError):
