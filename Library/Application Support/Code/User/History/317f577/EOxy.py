...
failure_flags: np.ndarray = np.zeros(N_TRIALS, dtype=bool)
...
for trial in range(N_TRIALS):
    ...
    for day in range(DAYS):
        ...
        g = g + float(np.random.normal(0.0, sigma_G))
...
lambda_mean: float  # type: ignore[assignment]
...
summary = {
    ...
    "total_weak_days": int(np.sum(weak_days_counts)), # pyright: ignore[reportUnknownMemberType] # pyright: ignore[reportUnknownArgumentType]
...
days: np.ndarray = np.arange(DAYS)
