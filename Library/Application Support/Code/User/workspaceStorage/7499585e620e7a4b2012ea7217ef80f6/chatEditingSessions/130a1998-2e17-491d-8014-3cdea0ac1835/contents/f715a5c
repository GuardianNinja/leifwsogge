...
# -------------------------
# Pre-allocate arrays
lambda_all = np.zeros((N_TRIALS, DAYS))
G_all = np.zeros((N_TRIALS, DAYS))
v_all = np.zeros((N_TRIALS, DAYS))
desat_counts: np.ndarray = np.zeros(N_TRIALS, dtype=int)
saturation_counts = np.zeros(N_TRIALS, dtype=int)
weak_days_counts = np.zeros(N_TRIALS, dtype=int)
failure_flags = np.zeros(N_TRIALS, dtype=bool)

for trial in range(N_TRIALS):
    g: float = float(G_init)
    v: float = float(v_init)
    e_t: float = 0.0
    ...
    for day in range(DAYS):
        ...
        g *= (1.0 + G_drift_frac_per_day)
        g += np.random.normal(0.0, sigma_G)
        ...
        # Compute Lambda
        lam = compute_lambda(Omega, R, alpha, G_factor_eff, float(v), t_p)
        ...
        v: float = v + float(np.random.normal(0.0, sigma_v))  # type: ignore[attr-defined, arg-type]
...
lambda_min = np.min(lambda_all)
lambda_max = np.max(lambda_all)
...
summary = {
    ...
    "total_saturation_events": int(np.sum(saturation_counts)),
    "total_desat_events": int(np.sum(desat_counts)),
    "total_weak_days": int(np.sum(weak_days_counts)),
    "failures": int(np.sum(failure_flags)), # pyright: ignore[reportUnknownMemberType]
}
...
lambda_median = np.median(lambda_all, axis=0)
lambda_p10 = np.percentile(lambda_all, 10, axis=0)
lambda_p90 = np.percentile(lambda_all, 90, axis=0)
days = np.arange(DAYS)
