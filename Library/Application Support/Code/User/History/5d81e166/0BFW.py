...
def stability_time_series(states: List[ShellState], days: int = 3650):
    ...
    overall: np.ndarray = per_shell.mean(axis=0) # pyright: ignore[reportUnknownMemberType]
    return t, overall, per_shell

def stability_time_series(
    states: List[ShellState], days: int = 3650
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    t: np.ndarray = np.arange(0, days + 1)
    per_shell: np.ndarray = np.zeros((len(states), len(t)))
    for i, s in enumerate(states):
        if s.excess_fraction == 0:
            per_shell[i, :] = 1.0
        else:
            per_shell[i, :] = np.exp(-s.lambda_daily * t)
    overall: np.ndarray = per_shell.mean(axis=0)
    return t, overall, per_shell
...
def run_scenario(name: str, counts: List[int], k: float = 3.0, days: int = 3650):
    ...
    def first_below(series: "np.ndarray", thresh: float) -> Optional[int]:
        idx = np.where(series < thresh)[0] # pyright: ignore[reportUnknownMemberType]
