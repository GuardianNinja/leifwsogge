...
def simulate_ash_cloud( # pyright: ignore[reportUnknownParameterType]
    grid_size: int = 100,
    time_steps: int = 50,
    ...
    ash = np.zeros((grid_size, grid_size)) # pyright: ignore[reportUnknownVariableType, reportUnknownMemberType]
    x0, y0 = eruption_center
    ash[x0, y0] = eruption_strength

    for _ in range(time_steps):
        shifted: np.ndarray = np.ndarray  # type: ignore
        shifted = np.roll(ash, shift=(int(wind_vector[0]), int(wind_vector[1])), axis=(0, 1)) # pyright: ignore[reportUnknownMemberType]
        ash = shifted * (1 - settling_rate)

    return ash # pyright: ignore[reportUnknownVariableType]
