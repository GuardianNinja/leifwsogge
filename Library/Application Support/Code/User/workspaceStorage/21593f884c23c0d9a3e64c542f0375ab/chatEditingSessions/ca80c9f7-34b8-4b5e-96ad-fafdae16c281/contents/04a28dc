...
class TestSpongeService(unittest.TestCase):
    def setUp(self):
        ...
        # Assign notifier_callback only if attribute exists and is not None (for compatibility with DiamondFirewall definition)
        if hasattr(self.firewall, "notifier_callback") and self.firewall.notifier_callback is not None:
            self.firewall.notifier_callback = cb
        elif hasattr(self.firewall, "set_notifier_callback"):
            self.firewall.set_notifier_callback(cb)
